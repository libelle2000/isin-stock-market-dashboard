...
describe('StockDataRepository', () => {
  ...
  beforeEach(() => {
    ...
    // Mock StockDataMapper.fromApiResponse to return a StockData instance
    (StockDataMapper.fromApiResponse as jest.Mock).mockImplementation((isin) => {
      return new StockData(isin, []);
    });
  });
  ...
  describe('getStockDataByIsin', () => {
    it('should return stock data when it exists in the cache', async () => {
      ...
      // Check that StockDataMapper.fromApiResponse was called with the correct arguments
      expect(StockDataMapper.fromApiResponse).toHaveBeenCalledWith(isin, mockResponse);
